...
function BigAvatar({ name }: { name: string }) {
    ...
    return (
        <div
            className="w-14 h-14 rounded-full flex items-center justify-center text-white text-xl font-bold select-none ring-[3px] ring-white shadow-md"
    ...
    const PwField = ({ label, value, onChange, show, onToggle, placeholder }: {
        ...
        <div>
            ...
            <div className="mt-1 relative">
                <input type={show ? "text" : "password"} className="input h-8 text-xs pr-10"
            ...
            >
                ...
                <div className="relative h-20 shrink-0"
                    ...
                    <button onClick={onClose}
                        className="absolute top-2.5 right-2.5 h-7 w-7 rounded-full bg-white/20 hover:bg-white/35 flex items-center justify-center text-white transition-colors">
                ...
                <div className="pt-9 pb-3 px-5 border-b flex items-end justify-between gap-3" style={{ borderColor: "var(--border)" }}>
                    <div>
                        <div className="text-base font-bold text-slate-900 leading-tight">{user?.username ?? "—"}</div>
                        <div className="text-xs text-slate-500 mt-0.5">{user?.email ?? "—"}</div>
                    </div>
                    <div className="flex items-center gap-1.5 mb-0.5 flex-wrap justify-end">
                        <span className={`inline-flex px-2 py-0.5 rounded-full text-[11px] font-semibold ${ROLE_COLOUR[user?.role ?? ""] ?? "bg-slate-100 text-slate-600"}`}>
                            {user?.role ?? "—"}
                        </span>
                        <span className={`inline-flex px-2 py-0.5 rounded-full text-[11px] font-semibold ${STATUS_COLOUR[String(user?.status ?? "")] ?? "bg-slate-100 text-slate-600"}`}>
                            {String(user?.status ?? "—")}
                        </span>
                    </div>
                </div>
                ...
                <div className="flex flex-1 overflow-hidden">
                    ...
                    {/* Left nav */}
                    <nav className="w-36 shrink-0 border-r bg-[#F8FAFD] py-2 flex flex-col gap-0.5" style={{ borderColor: "var(--border)" }}>
                        {NAV_ITEMS.map((item) => (
                            <button
                                key={item.id}
                                onClick={() => setTab(item.id)}
                                className={`flex items-center gap-2 w-full text-left px-3 py-2 text-xs font-medium transition-colors rounded-lg mx-1 ${tab === item.id
                    ...
                    <div className="flex-1 overflow-y-auto p-4">
                        ...
                        {tab === "info" && (
                            <div>
                                <h3 className="text-xs font-semibold text-slate-900 uppercase tracking-wide mb-3">Account Information</h3>
                                <div className="px-3 rounded-[8px] border overflow-hidden" style={{ borderColor: "var(--border)" }}>
                        ...
                        {tab === "edit" && (
                            <div className="space-y-3">
                                ...
                                <div className="rounded-[8px] border overflow-hidden" style={{ borderColor: "var(--border)" }}>
                                    <div className="px-3 py-1.5 bg-[#F8FAFD] border-b text-[10px] font-semibold text-slate-500 uppercase tracking-wide" style={{ borderColor: "var(--border)" }}>
                                        Admin-managed fields (read only)
                                    </div>
                                    <div className="divide-y px-3" style={{ borderColor: "var(--border)" }}>
                                        <InfoRow label="Email" value={user?.email} />
                                        <InfoRow label="Role" value={user?.role} />
                                ...
                                <button className="btn-primary h-8 text-xs w-full" onClick={onSaveEdit} disabled={editSaving}>
                            ...
                            <div className="space-y-3">
                                ...
                                {newPw.length > 0 && (
                                    <div className="flex gap-1 -mt-1.5">
                                        {[...Array(4)].map((_, i) => (
                                            <div key={i}
                                                className={`h-1 flex-1 rounded-full transition-all ${i < Math.min(4, Math.floor(newPw.length / 3))
                                ...
                                <button className="btn-primary h-8 text-xs w-full" onClick={onChangePassword} disabled={pwSaving}>
